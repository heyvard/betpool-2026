...
function lagClient(opts: {
    ...
    const query = jest.fn(async (sql: string, params?: unknown[]) => {
        if (sql.includes("interval '2 hours'") && sql.includes('feed_posts fp')) return { rows: opts.kampWindow }
...
it('backfiller kamp-poster med historisk created_at = sluttidspunkt', async () => {
    ...
    const res = await backfillFeed(client, 2, new Date('2026-06-20T09:00:00Z'))

    expect(res.kampposter).toBe(1)
    expect(res.morgenrapporter).toBe(0) // stille dager (0 ferdige kamper «i går»)
