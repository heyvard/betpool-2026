...
// Toggler en emoji-reaksjon på en post, med optimistisk oppdatering.
export function UseMutateFeedReaksjon() {
    const queryClient = useQueryClient()
    const authedFetch = useAuthedFetch()

    return useMutation<void, Error, { postId: string; emoji: string }, { forrige?: FeedRespons }>({
        mutationFn: async ({ postId, emoji }) => {
            const response = await authedFetch(`/api/v1/feed/${postId}/reactions`, {
                method: 'POST',
            ...
            queryClient.setQueryData<FeedRespons>(FEED_QUERY_KEY, (gammel) => {
                ...
                return {
                    posts: gammel.posts.map((p) =>
                        p.id === postId ? { ...p, reactions: toggleReaksjon(p.reactions, emoji) } : p,
                    ),
                }
            })
            return { forrige }
